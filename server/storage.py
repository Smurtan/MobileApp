...
class Storage:
    ...
    def registration(self, params):
        ...
        # params['password'] = self.hash_password(params['password'])
        sql_request = """
            INSERT INTO
                users
            ...
            VALUES 
                ('%s', '%s', '%s', '%s', '%s', '%s');
    """ % (params['lastName'], params['firstName'], params['middleName'], params['phone'], params['login'],
           params['password'])
        print(sql_request)
    ...
    def enter(self, params):
        ...
        WHERE
            login = '%s';""" % params['login']
        answer = self.read_query(connect, sql_request)[0][0][0]
        print(answer)
    ...
    def loadReview(self, params):
        ...
        sql_request = """
                    ...
                    WHERE
                        u.login = %s AND 
                        u.id_user = r.id_to;""" % params['user']
        answer = self.read_query(connect, sql_request)
        print(answer)
    ...
    def appendRequest(self, params):
        ...
        sql_request = """
                    ...
                    VALUES 
                        (%s, '%s', '%s', '%s');
            """ % (user, params['From'], params['To'], params['date'])
        answer = self.append_query(connect, sql_request)
    ...
    def loadStoryTravels(self, params):
        ...
        sql_request = """
                            ...
                            FROM
                                travels AS t
                            WHERE
                                (t.`id_driver` = %s OR t.`id_passager` LIKE '%s') AND t.id_condition = 2;""" % (params['user'], '%' + params['user'] + '%')
        answer = self.read_query(connect, sql_request)[0]
